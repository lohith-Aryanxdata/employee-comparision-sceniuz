...
import { Download, Trophy, BarChart3, Target, BrainCircuit, Medal } from "lucide-react";
...
function LeaderboardRow({ e, index }) {
    ...
    <div style={{ position: 'relative', minHeight: '100vh', padding: '32px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '16px' }}>
        <div>
          ...
          <p style={{ ontSize: '20px', color: '#ffffff', marginTop: '8px' }}>Platform-wide skill assessment insights</p>
        </div>
        <button
      ...
      <ChartCard title="Top Performers Leaderboard" subtitle="Ranking the best skill scores platform-wide" icon={Trophy} loading={l4}>
        {topData.length > 0 ? (
          <div>
            ...
            {/* Podium rows (top 3) */}
            {topData.filter(e => e.rank <= 3).map((e, idx) => (
              <LeaderboardRow key={e.email} e={e} index={idx} />
            ))}
